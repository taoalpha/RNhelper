<?php
    ...
    function getUser()
    {
        ...
        try {
            ...
            $_COOKIE['username'] = $userinfo->username;
            $currentStatus = 'Successfully get user name';

            return $_COOKIE['username'] ;
        } catch (EDAMSystemException $e) {
            if (isset(EDAMErrorCode::$__names[$e->errorCode])) {
                $lastError = 'Error getting username: ' . EDAMErrorCode::$__names[$e->errorCode] . ": " . $e->parameter;
            } else {
                $lastError = 'Error getting username: ' . $e->getCode() . ": " . $e->getMessage();
            }
            ...
            $lastError = 'Error getting username: ' . $e->getMessage();
        }
     ...
     * *
     */
    function addNote($title,$content,$tags,$surl)
    {
        global $lastError, $currentStatus;

        $noteAttr = new NoteAttributes();
        $noteAttr->sourceApplication = "RNhelper";
        $noteAttr->sourceURL = $surl;
        $note = new Note();
        $note->title = $title;
        ...
        try {
            ...
            $userinfo = $client->getNoteStore()->createNote($note);
            $currentStatus = 'Successfully create new note';

            return TRUE;
        } catch (EDAMSystemException $e) {
            if (isset(EDAMErrorCode::$__names[$e->errorCode])) {
                $lastError = 'Error : ' . EDAMErrorCode::$__names[$e->errorCode] . ": " . $e->parameter;
            } else {
                $lastError = 'Error creating note: ' . $e->getCode() . ": " . $e->getMessage();
            }
        } catch (EDAMUserException $e) {
            if (isset(EDAMErrorCode::$__names[$e->errorCode])) {
                $lastError = 'Error creating note: ' . EDAMErrorCode::$__names[$e->errorCode] . ": " . $e->parameter;
            } else {
                $lastError = 'Error creating note: ' . $e->getCode() . ": " . $e->getMessage();
            }
        } catch (EDAMNotFoundException $e) {
            ...
            $lastError = 'Error creating note: ' . $e->getMessage();
        }
    ...
    function listNotebooks()
    {
        ...
        try {
            ...
            $username = $client->getUserStore()->getUser();
            $result = array();
            if (!empty($notebooks)) {
    ...
    function getCallbackUrl()
    {
        ...
        $thisUrl = preg_replace('/[?&]action=[^&]*/i', '', curPageURL());
        // use count($_GET) instead next
        if(isset($_GET["url"])){
            $thisUrl .= "&action=callback";
        }else{
            $thisUrl .= "?action=callback";
